...
GtkWidget *textHistory, *textMean, *text_view_edit, *text_view_add; // cua so text
GtkWidget *searchEntry; // o nhap tu tim kiem
GtkWidget *entry1, *entry2; // entry1: add ; entry2: edit
GtkWidget *window, *windowAdd, *windowEdit, *windowAbout; // cac cua so
GtkListStore *list;
GtkTreeIter iter;

BTA *book;

char history[MEAN_SIZE], suggest[WORD_SIZE]; //lich su tim kiem va tu goi y
int flag, sai = 1; //bien check
...
void on_key_pressed(GtkWidget *widget, GdkEventKey *event, gpointer userData) //Nhap du lieu tu nguoi dung
{
    char word[WORD_SIZE];
    int x;
    strcpy(word, gtk_entry_get_text(GTK_ENTRY(searchEntry)));  //copy du lieu tu searchEntry vao bien "word"
    x = strlen(word);
    if(x == 0) sai = 1;  
